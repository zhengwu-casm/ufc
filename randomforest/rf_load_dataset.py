...
# load dataset
def load_data(filename, add_building_count, data_separation=None):
    ...
    if data_separation is not None:
        if len(data_separation) == 3:
            return separating_dataset(data, data_separation, add_building_count)
    else:
        return constructing_dataset(data, add_building_count, False, 1)[:7]

# split dataset into train dataset, validate dataset, test dataset
def separating_dataset(dataset, data_separation, add_building_count):
    ...
    train_data = constructing_dataset(train_dic, add_building_count, True, 1)

    return train_data[:5], \
           constructing_dataset(val_dic, add_building_count, False, 2, train_data[2], train_data[3])[:4], \
           constructing_dataset(test_dic, add_building_count, False, 3, train_data[2], train_data[3])[:4]

# constructing input data for random forest
def constructing_dataset(dataset, add_building_count, is_oversampling, data_type, mean_feature=0, std_feature=1):
    ...
    for k in dataset_resample:
        ...
        if add_building_count:
            new_vertice_features = [subobject_size] + vertice_features
        else:
            new_vertice_features = vertice_features
        if subobject_size < 2:
